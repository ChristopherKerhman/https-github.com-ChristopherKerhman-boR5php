...
// Fin affichage des armes
  ...
  if ($key['niveauMage'] > 0) {
   include 'affichages/AffichageSortGenerique.php';
 }
echo '</ul>';
// Fin de la fiche
  }
...
foreach ($dataWeaponV as $gun) {
  if ($gun['type'] == 1) {
    echo '<li><strong>Statistique</strong></li><li><strong>Arme de mêlée : </strong> '.$gun['nomArme'].' <strong>Puissance anti-véhicule :</strong> '.$gun['puissance'].$typeDe[$dataVehicule[0]['DC']]['de'].' <strong>Puissance anti-personnelle :
    </strong> '.(2 * $gun['puissance']).$typeDe[$dataVehicule[0]['DC']]['de'].'  <strong>Sort :</strong>'.$yes[$gun['sort']]['texte'];
    }
  if ($gun['type'] == 2) {
    echo '<li><strong>Arme de tir : </strong> '.$gun['nomArme'].'&thinsp;<strong>Portée :</strong>'.$gun['rangeMax'].' " <strong>Puissance anti-véhicule :</strong> '.$gun['puissance'].$typeDe[$dataVehicule[0]['DC']]['de'].'
    <strong>Puissance anti-personnelle :</strong> '.(2 * $gun['puissance']).$typeDe[$dataVehicule[0]['DC']]['de'].'
    <strong>Sort :</strong>'.$yes[$gun['sort']]['texte'].'
    <strong>Arme lourde :</strong>'.$yes[$gun['lourde']]['texte'].'
    <strong>Assaut :</strong>'.$yes[$gun['assaut']]['texte'];
    if ($gun['couverture'] == 1) {
  ...
  if ($gun['type'] == 3) {
    echo '<li><strong>Arme explosive : </strong> '.$gun['nomArme'].'&thinsp;<strong>Portée :</strong>'.$gun['rangeMax'].' " <strong>Puissance anti-véhicule :</strong> '.$gun['puissance'].$typeDe[$dataVehicule[0]['DC']]['de'].'
    <strong>Puissance anti-personnelle :</strong> '.(2 * $gun['puissance']).$typeDe[$dataVehicule[0]['DC']]['de'].'
    <strong>Sort :</strong>'.$yes[$gun['sort']]['texte'].'
    <strong>Arme lourde :</strong>'.$yes[$gun['lourde']]['texte'].'
    <strong>Assaut :</strong>'.$yes[$gun['assaut']]['texte'];
