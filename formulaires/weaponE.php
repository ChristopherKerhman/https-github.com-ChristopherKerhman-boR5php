...
<h3>Création d'une nouvelle arme explosive</h3>
<article class="conteneur_col">
<form action="gestionDB/record/weaponE.php" method="post">
...
<div class="conteneur_col box marge">
...
<div class="conteneur_col box">
  <label for="explosif">Gabarit</label>
  <select id="explosif" class="sizeInpute" name="explosif">
<?php
include 'stockage/gabarit.php';
foreach ($gabarit as $key) {
  echo '<option value="'.$key['index'].'">'.$key['texte'].'</option>';
}
 ?>

  </select>
  <label for="de">Dé de dommage</label>
  <select id="de" class="sizeInpute" name="dExplosive">
<?php
